...
# class AISC_ClassificationModel(torch.nn.Module):
#     def __init__(self, path_to_weights, query_label):
#
#         super().__init__()
#         self.feat_extract = DenseNet121()
#         self.classifier = torch.nn.Linear(self.feat_extract.output_size, 2)
#         self.query_label = query_label
#
#         # load the model from the checkpoint
#         state_dict = torch.load(path_to_weights, map_location='cpu')
...
class SkinClassificationModel(torch.nn.Module):
...
class AISC_ClassificationModel(torch.nn.Module):
    def __init__(self, num_classes=2,query_label=0):
        super().__init__()
        self.query_label = query_label
        self.feat_extract = DenseNet121()
        self.classifier = torch.nn.Linear(self.feat_extract.output_size, num_classes)
    ...
    def forward(self, x):
        features = self.feat_extract(x)
        logits = self.classifier(features)
        return logits[:, self.query_label]