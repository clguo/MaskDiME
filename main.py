...
def merge_and_compute_overall_metrics(args, device):
    ...
    for chunk in range(args.num_chunks):
        ...
        for k in ['bkl', 'l_1', 'FVA', 'MNAC']:
            ...
            summary['class-cor']['cf-cor'][k] += chunk_summary['class-cor']['cf-cor'][k] * \
                                                 chunk_summary['class-cor']['cf-cor']['n']
            summary['class-cor']['cf-inc'][k] += chunk_summary['class-cor']['cf-inc'][k] * \
                                                 chunk_summary['class-cor']['cf-inc']['n']
            summary['class-inc']['cf-cor'][k] += chunk_summary['class-inc']['cf-cor'][k] * \
                                                 chunk_summary['class-inc']['cf-cor']['n']
            summary['class-inc']['cf-inc'][k] += chunk_summary['class-inc']['cf-inc'][k] * \
...
def main():
    ...
    if args.dataset in ['CelebAMV', 'CelebAMT', 'CelebA']:
        classifier = ClassificationModel(args.classifier_path, args.query_label).to(dist_util.dev())
        classifier.eval()


    # Count parameters (insert here)
    def count_parameters(model):
        return sum(p.numel() for p in model.parameters() if p.requires_grad)

    num_params_model = count_parameters(model)
    num_params_classifier = count_parameters(classifier)
    total_params = num_params_model + num_params_classifier

    print(f"\n[📊 PARAMETER SUMMARY]")
    print(f"Diffusion model parameters: {num_params_model / 1e6:.2f} M")
    print(f"Classifier parameters:      {num_params_classifier / 1e6:.2f} M")
    ...
    elif args.method == "maskdime":
        if args.mask == True:
            sample_fn = get_MaskDiME_sampling_CelebA(use_sampling=args.use_sampling_on_x_t)
    ...
    for idx, (indexes, img, lab) in enumerate(loader):
        ...
        for jdx, classifier_scale in enumerate(classifier_scales):
            ...
            peak_memory_ce = torch.cuda.max_memory_allocated() / (1024 ** 2)
            print(f"⚙️ [Counterfactual] Peak GPU memory used: {peak_memory_ce:.2f} MB")
    ...
    for k in ['bkl', 'l_1']:
        summary['class-cor']['cf-cor'][k] = mean(
            stats[k][(stats['label'] == stats['pred']) & (stats['target'] == stats['cf pred'])])
